...
const LoginActivity = () => {
  ...
  // Calculate exactly a 6-month window
  const today = new Date();
  ...
  return (
    ...
    <div className="flex flex-1 w-full h-full min-h-[6rem] rounded-sm dark:from-neutral-900 dark:to-neutral-800 to-neutral-100 p-3 overflow-hidden items-center justify-center">
      {/* 
        This wrapper forces the child SVG to scale responsively rather than maintaining 
        a fixed width that causes horizontal scrolling. 
      */}
      <div className="w-full max-w-full overflow-hidden [&_svg]:w-full [&_svg]:max-w-full [&_svg]:h-auto">
        <ActivityCalendar
          data={data}
          blockSize={14}
          blockMargin={4}
          ...
          renderBlock={(block, activity) => (
            <MuiTooltip
              ...
              arrow
              placement="top"
              key={activity.date} // Added key to prevent React warning
            >
              {block}
            </MuiTooltip>
          )}
          renderColorLegend={(block, level) => {
            ...
            return (
              <MuiTooltip
                ...
                arrow
                placement="top"
                key={`legend-${level}`} // Added key to prevent React warning
              >
                {block}
              </MuiTooltip>
            );
