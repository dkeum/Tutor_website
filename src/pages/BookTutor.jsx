...
const BookTutor = () => {
  ...
  return (
    <div className="min-h-screen w-full text-[#101b30] flex relative overflow-x-hidden bg-[#f9f9ff]">
      {/* Dynamic Math Matrix Background Grid Pattern */}
      <style>{`
        .math-grid {
          background-size: 40px 40px;
          background-image: 
            linear-gradient(to right, rgba(98, 0, 238, 0.05) 1px, transparent 1px),
            linear-gradient(to bottom, rgba(98, 0, 238, 0.05) 1px, transparent 1px);
        }
      `}</style>
      
      <div className="absolute inset-0 math-grid pointer-events-none z-0" />

      {/* Global Master Fixed Header Bar: Placed at the top level over both viewports */}
      <div className="fixed top-0 left-0 right-0 h-20 z-50">
        <NavbarLoggedIn />
      </div>

      {/* Navigation Left Sidebar Panel Layout: Set lower than Navbar stacking order */}
      <div className="relative z-40">
        <Sidebar />
      </div>

      {/* Main Core Content Shell Wrap */}
      <div className="flex-1 pl-64 xl:pl-0 flex flex-col min-h-screen relative z-10 pt-20">
              ...
              <div className="lg:col-span-5 space-y-6 flex flex-col justify-center text-left">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-[0_4px_20px_rgba(98,0,238,0.06)] border border-[#e0e8ff] transition-transform duration-300 hover:-translate-y-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1 bg-[#F8F9FE] p-3 rounded-xl text-center">
                      <span className="block text-[#4800b2] font-bold text-base">2k+</span>
